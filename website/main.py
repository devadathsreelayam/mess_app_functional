...
def delete_inmate(inmate_id):
    try:
        ...
        if success:
            ...
            return redirect(url_for('manage_inmates')), jsonify({"error": "Inmate not found"}), 404
    except Exception as e:
...
@app.route('/bills')
def bills():
    return render_template('bills.html', app_start_date=APP_STARTING_DATE)
